...
// FIXME just trying to get the behaviour of this correct
// probably should use a compare and swap - not sure if it's necessary as I'm not sure if this all runs under interrupt still
// MT 12/1/12
// UPDATE: this code doesn't seem to get hit - might not be used anymore anyway? (MT 14/1/12)
Boolean OTAtomicSetBit(Boolean *ioBool, int inBit)
{
	Boolean	res;
	if (!*ioBool)
	{
